...
*/
void Subsume(vector<CBlob*> &RegionData,
			int HighRegionNum,
			int* SubsumedRegion,
			CBlob* blobHi,
			CBlob* blobLo,
			...
			int LoNum)
{
	...
	if( findmoments )
	{
		blobLo->u10+=blobHi->u10;	
		blobLo->u01+=blobHi->u01;	
		blobLo->u20+=blobHi->u20;	
		blobLo->u02+=blobHi->u02;	
		blobLo->u11+=blobHi->u11;
	}
	// Make sure no region still has subsumed region as parent
	vector<CBlob*>::iterator it = (RegionData.begin() + HiNum + 1);
